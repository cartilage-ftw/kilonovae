...
def calc_z(p: float, t: float, delta: np.array):
    ...
    called Sobolev points/planes. Note that these "planes" are curved due to special relativistic effects.
...
@partial(jax.tree_util.register_dataclass, 
         data_fields=['polar_opening_angle', 'observer_angle', 'v_phot', 'v_max', 't_d', 'T_phot_comoving'],
...
class Photosphere:
    ...
    T_phot_comoving: float # the temperature of the photosphere in the comoving frame
    ...
    def tau(self, line, p, z, r, v, is_polar):
        ...
        tau_ = jnp.where(is_polar,
                        corr*line.tau_interp_polar((v/c)),
                        corr*line.tau_interp_eq((v/c)))
        # check if it's in the line forming regions
    ...
    @jax.jit
    def source_function(self, nu, v, z, r, g_u, g_l, n_u_grid, n_l_grid, v_grid, nu_0, continuum, mode='dilute-LTE'):
        """
        The source function, defined as the ratio of the emissivity to the absorption coefficient
        has the following form.
        """
        if mode == 'dilute-LTE':
            T = self.T_phot_comoving
            # Lorentzian difference in velocities
            delta_v = ((v - self.v_phot)/c) / (1 - v*self.v_phot/c**2)
            delta_factor = 1/(1/jnp.sqrt(1 - delta_v**2) * (1+delta_v))
    ...
    def S(self, nu, p, z, r, r_min, r_max, v, sob_esc_prob, g_u, g_l, n_u_grid, n_l_grid, v_grid, nu_0, continuum, mode='level-pop'):
        ...
        mu = z / r
        beta = v/c
        gamma = lambda beta: 1/(jnp.sqrt(1 - beta**2))
        delta_factor = 1/(gamma(beta) * (1 - mu*beta))
        if mode == 'level-pop':
            # the level populations are given from the NLTE calculation
            return jnp.where(outside_photosphere | occulted_region,
                            1e-10,
                            delta_factor**2 * self.source_function(nu, v, z, r, g_u, g_l, n_u_grid, n_l_grid, v_grid, nu_0, continuum)
                            )
        else:
            return jnp.where(outside_photosphere | occulted_region,
                        1e-10,
    ...
    def is_polar(self, x, y, z, r):
        ...
        # define a unit vector along the cone axis
        nx = jnp.sin(beta)
        nz = jnp.cos(beta)
        # take the dot product with the cone axis
        dot = (x*nx + z*nz) / jnp.maximum(r, 1e-12) # for numerical safety
        # if this is within the full opening angle, then yes it's polar ejecta
