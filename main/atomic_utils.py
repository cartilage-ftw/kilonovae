...
def computeLTEpop(level_data: pd.DataFrame, T: Quantity):
	energies = level_data['Level (cm-1)'].to_numpy()
	# convert to Joules
	energies_J = h*c*(energies*100) * u.J
	
	boltzmann_factors = calc_boltzmann(energies_J, T)
	# partition function
	Z = np.sum(boltzmann_factors) * level_data['g'].to_numpy()
	# probability of occupation
	occupancies = boltzmann_factors/Z
	return occupancies
...
def partition_func(levels: pd.DataFrame, T: float):
	if not isinstance(T, Quantity):
		T = T * u.K
	boltzmann = calc_boltzmann(levels['Ek(cm-1)'].to_numpy() / u.cm, T)
